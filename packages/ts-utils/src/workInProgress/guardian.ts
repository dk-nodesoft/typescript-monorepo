...
const isArrayTypeValid = ([_, t]: ArrayTypeDef, value: unknown) =>
  Array.isArray(value) && value.every((el: unknown) => isTypeValid(t, el));

const isTupleTypeValid = (t: TupleTypeDef, value: unknown) =>
  Array.isArray(value) && t.every((g, i) => isTypeValid(g, value[i]));
...
const createOrInstanceOf =
  ...
  <TNew extends new (...args: any[]) => any>(t: TNew) =>
    createGuard<TPrev | (TNew extends new (...args: any[]) => infer V ? V : never)>([
      ...prevTypeDefinitions,
      [instanceMarker, t]
    ]);
...
export const isRecordOf = <T extends TypeDef>(t: T) =>
  createGuard<Record<PropertyKey, TypeDefType<T>>>([[recordMarker, t]]);
...
export const parserFor =
  ...
  (
    value: any
  ): T extends undefined ? GuardType<TGuard> | undefined : GuardType<TGuard> extends T ? T | undefined : never =>
    guard(value) ? value : undefined;

export const assertThat: <T extends any>(value: any, guard: Guard<T>, errorMessage?: string) => asserts value is T = <
  T extends any
>(
  value: any,
  guard: Guard<T>,
  ...
  if (!guard(value))
    throw new TypeError(errorMessage ?? `Type of '${value?.name ?? String(value)}' does not match type guard.`);
};
